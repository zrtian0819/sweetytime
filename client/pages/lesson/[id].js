...
export default function LessonDetail(props) {
	...
	const handleLike = (id) => {
		if (user) {
			...
			if (isLike == true) {
				axios
					.post(`${process.env.NEXT_PUBLIC}/api/lesson/likeDel/${id}`, data)
					.then((res) => {
						setIsLike(!isLike);
				...
				axios
					.post(`${process.env.NEXT_PUBLIC}/api/lesson/like/${id}`, data)
					.then((res) => {
						setIsLike(!isLike);
	...
	useEffect(() => {
		axios
			.get(`${process.env.NEXT_PUBLIC}/api/lesson/${id}`)
			.then((response) => {
	...
	useEffect(() => {
		axios
			.get(`${process.env.NEXT_PUBLIC}/api/lesson`)
	...
	useEffect(() => {
		axios
			.get(`${process.env.NEXT_PUBLIC}/api/lesson/student/${id}`)
			.then((response) => {
