...
const requestOtpToken = async (email) => {
	try {
		const response = await axios.post(
			`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/reset-password/request-otp`,
			{
				email,
			}
		);
...
const resetPassword = async (email, newPassword, otp) => {
	try {
		const response = await axios.post(
			`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/reset-password/reset-password`,
			{
				email,
				newPassword,
				otp,
			...
			<div className={styles['WGS-loginCardPW']}>
				...
				<form className={styles['WGS-loginForm']} onSubmit={handleResetPassword}>
					...
					<div>
						...
						<button type="submit" className={styles['WGS-loginBtn']}>
							重設密碼
						</button>
					</div>
				</form>
