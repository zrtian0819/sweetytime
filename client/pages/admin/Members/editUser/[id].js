...
const EditUser = () => {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			formData.append('activation', userData.activation ? '1' : '0');
			...
			const response = await axios.put(
				`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/userRoutes/edit/${id}`,
				formData,
				{
					headers: {
			...
			setError(err.response?.data?.message || err.message || '更新失敗，請檢查後再試');
		} finally {
			setIsSubmitting(false);
		}
	};
